...
import { 
  Target, Search, Phone, Mail, ChevronRight, ChevronDown, Database, FileText, Sparkles
} from 'lucide-react'
...
export function LeadsPageClient({ initialLeads, services }: LeadsPageClientProps) {
  ...
  // Default selection: first available voice agent, otherwise first available agent in tools list
  ...
  const isVoiceAgent = selectedService?.type?.toLowerCase() === 'voice'

  // Search filter matching name, email, or phone
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-white/5">
        ...
        {/* Dropdown Selector */}
        {services.length > 0 && (
          <div className="relative min-w-[240px] group self-start md:self-auto">
      ...
      {isVoiceAgent ? (
        ...
        <div className="bg-[#0f1117]/85 border border-white/5 backdrop-blur-md rounded-2xl p-16 text-center flex flex-col items-center justify-center gap-4 min-h-[400px] shadow-2xl relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-violet-600/5 to-transparent pointer-events-none" />
          <div className="w-16 h-16 rounded-full bg-zinc-800/60 border border-white/5 flex items-center justify-center text-3xl mb-4 text-zinc-500 shadow-inner">
            <Database className="w-8 h-8" />
          </div>
          <h3 className="text-xl font-bold text-white tracking-wide">CRM Not Applicable</h3>
          <p className="text-zinc-400 text-sm max-w-lg mt-1 mb-6 leading-relaxed">
            This autonomous agent does not capture traditional CRM leads. Data outputs for OCR and processing agents are routed directly to your connected webhooks, databases, or returned via API.
          </p>
        </div>
      )}
    </div>
