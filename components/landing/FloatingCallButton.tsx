...
function getFloatingStatusCopy({
  ...
  if (isConnecting) {
    return "Connecting to AI agent. This can take 5-10 seconds.";
  }

  if (isConnected && isSpeaking) {
    return "AI agent is speaking now";
  }

  if (isConnected) {
    return "You are live with the AI agent";
  }
  ...
  return (
    <div className="fixed bottom-[28px] right-[28px] z-[999] flex flex-col items-end gap-[12px]">
          ...
          <motion.div
            ...
            className={`max-w-[240px] rounded-[14px] border px-[14px] py-[10px] text-right font-sans text-[13px] leading-[1.5] shadow-[0_10px_30px_rgba(0,0,0,0.35)] ${
              isConnected
                ...
                : "border-[rgba(139,92,246,0.24)] bg-[rgba(19,2,36,0.92)] text-[#C4B5FD]"
            }`}
          >
            {statusCopy}
          </motion.div>
      ...
      <button
        ...
        aria-label={isConnected ? "End AI call" : "Connecting to AI call"}
      >
        {isConnecting ? (
