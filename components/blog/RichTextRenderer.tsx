    ...
    if (match[2] && match[3]) {
      parts.push(
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="text-[#A78BFA] underline transition-colors hover:text-[#F5F3FF]"
        >
          {match[2]}
        </a>
      );
      ...
      parts.push(
        <strong key={`${keyPrefix}-bold-${index}`} className="font-semibold text-[#F5F3FF]">
          {match[4]}
        </strong>
      );
    } else if (match[5]) {
...
function renderTextBlock(block: string, key: string) {
  ...
  if (cleanLines.length === 1 && /^###\s+/.test(cleanLines[0])) {
    return (
      <h3 key={key} className="font-display text-[24px] font-semibold text-[#F5F3FF]">
  ...
  if (cleanLines.length === 1 && /^##\s+/.test(cleanLines[0])) {
    return (
      <h2 key={key} className="font-display text-[28px] font-semibold text-[#F5F3FF]">
        {renderInline(cleanLines[0].replace(/^##\s+/, ""), key)}
      </h2>
    );
  }
  ...
  if (cleanLines.length === 1 && /^#\s+/.test(cleanLines[0])) {
    return (
      <h1 key={key} className="font-display text-[32px] font-bold text-[#F5F3FF]">
        {renderInline(cleanLines[0].replace(/^#\s+/, ""), key)}
      </h1>
    );
  }
  ...
  return (
    <p key={key} className="font-sans text-[16px] leading-[1.8] text-[#A8A0C0]">
