...
export function generateInvoicePdf(invoiceData: {
  ...
  items: { quantity: number; description: string; rate: string; amount: string }[]
}): Buffer {
  const {
    invoiceNumber,
    date,
  ...
  const logoBuffer = fs.readFileSync(path.join(publicDir, 'logo-transparent.png'))
  const signatureBuffer = fs.readFileSync(path.join(publicDir, 'signature.png'))
  const watermarkBuffer = fs.readFileSync(path.join(publicDir, 'trident.png'))
