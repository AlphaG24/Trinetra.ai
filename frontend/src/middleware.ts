...
function applySecurityHeaders(response: NextResponse): NextResponse {
  ...
  const cspHeaderValue = `
    ...
    connect-src 'self' http://localhost:8000 http://127.0.0.1:8000 ws://localhost:7880 ws://127.0.0.1:7880 http://localhost:7880 http://127.0.0.1:7880 https://*.trycloudflare.com https://*.onrender.com https://*.supabase.co wss://*.supabase.co https://formspree.io https://*.googleapis.com https://*.razorpay.com https://*.daily.co wss://*.daily.co wss://*.wss.daily.co https://*.pluot.blue wss://*.pluot.blue https://raw.githubusercontent.com blob: data: stun: turn:;
    media-src 'self' blob: data: https://api.sarvam.ai https://api.elevenlabs.io;
...
export async function middleware(request: NextRequest) {
  ...
  // Unauthenticated users attempting protected routes
  if (!user && (isDashboardRoute || isAdminRoute || isPartnerDashboard || currentPath === '/consent')) {
  ...
  // Authenticated users
  if (user) {
    // 1. Fetch user role + onboarding status (do NOT include consented here —
    //    the column may not exist yet; consent is checked via consent_records below)
    ...
    // 3. Client Consent Gate — check consent_records table (source of truth)
    //    Runs only for client users on routes that require consent
    if (userRole === 'client') {
      const isConsentRoute = currentPath === '/consent'
      ...
      if (isConsentRoute || isProtectedClientRoute) {
    ...
    // 3. Admin user trying to access client dashboard
    if (isDashboardRoute && (userRole === 'super_admin' || userRole === 'admin')) {
      const url = request.nextUrl.clone()
      url.pathname = '/admin'
      return NextResponse.redirect(url)
    }

    // 4. Client onboarding guard — redirect to onboarding if not completed
    // Allow: /dashboard/onboarding itself, and API routes
    if (
      isDashboardRoute &&
      userRole === 'client' &&
      !onboardingComplete &&
    ...
    // 5. Prevent accessing onboarding if already completed
    if (
      currentPath.startsWith('/dashboard/onboarding') &&
      userRole === 'client' &&
      onboardingComplete
    ...
    // 6. Client user trying to access admin dashboard
    if (isAdminRoute) {
      if (!['admin', 'super_admin'].includes(userRole)) {
        ...
        try {
          ...
          console.error('[Security Log Error] Audit log insertion failed:', logErr)
        }
      ...
      // Super Admin System Route Gating
