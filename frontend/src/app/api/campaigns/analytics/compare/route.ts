...
export async function GET() {
  try {
    ...
    const response = await fetch(`${fastApiUrl}/api/campaigns/analytics/compare?organization_id=${profile.organization_id}`, {
      method: "GET",
      cache: "no-store"
    });
    ...
    if (!response.ok) {
      return NextResponse.json({ error: data.detail || "Failed to compare campaigns" }, { status: response.status });
    }
